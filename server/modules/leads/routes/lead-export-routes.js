...
function buildWorkspaceCsv(workspace) {
  ...
  workspace.companies.forEach((company) => {
    rows.push([
      ...
      company.address || '',
      company.phone || '',
      company.mapVerified ? 'true' : 'false',
      company.map?.placeId || '',
      (company.contactEmails || []).join(' | '),
      (company.emails || []).map((email) => email.sourceUrl).filter(Boolean).join(' | '),
      (company.evidence || []).map((item) => item.sourceUrl).filter(Boolean).join(' | '),
      company.contactEmailStatus || '',
...
function buildWorkspaceWorkbook(workspace) {
  ...
  appendJsonSheet(workbook, 'Companies', (workspace.companies || []).map((company) => ({
    ...
    mapPlaceId: company.map?.placeId || '',
    contactEmails: (company.contactEmails || []).join('; '),
    emailSources: (company.emails || []).map((email) => email.sourceUrl).filter(Boolean).join('; '),
    evidenceSources: (company.evidence || []).map((item) => item.sourceUrl).filter(Boolean).join('; '),
    contactEmailStatus: company.contactEmailStatus || '',
